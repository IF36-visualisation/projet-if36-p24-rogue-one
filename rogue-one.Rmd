...
1. Nous avons commencé par **observer les colonnes** de nos jeux de données. Nous avons pu isoler lesquels étaient susceptibles d'être utilisées.
...
On veut avoir une vue d'ensemble et donc nous allons visualiser la fréquence de voyageurs en 2022 sur la France métropolitaine.
...
gares_avec_pop <- gares %>%
  inner_join(frequentation , by = c("Nom" = "Nom de la gare"))
gares_separated <- gares_avec_pop %>%
  separate("Position géographique", into = c("Latitude", "Longitude"), sep = ", ") %>%
  mutate(across(c(Latitude, Longitude), as.numeric)) %>%
  rename(Freq_2022 = "Total Voyageurs 2022")
ggplot(data = france) +
  geom_sf(fill = "white", color = "black") +
  geom_point(data = gares_separated, aes(x = Longitude, y = Latitude, size=Freq_2022), color = "red", alpha = 0.7) +
  scale_size_continuous(
    range = c(1,8), 
  ) +
  theme_minimal() +
  labs(title = "Fréquentation par gare en 2022",
       x = "Longitude",
       y = "Latitude",
       size = "Fréquentation")
```
