...
| 14  | pourcentage\_ deplacement | Continue | \%                  | 3                 | Pourcentage sur le motif de déplacement des voyageurs          |
...
Les données sont importées dans R dans leur forme brute. On les manipule ensuite afin de créer nos propres tableaux. La section suivante concernera l'**Exploration de ces données**. On y détaillera les 3 grands axes que sont la Découverte, les Voyageurs et les Objets.
...
gares_clean <- gares %>%
  ...
  rename(Gare = "Nom", code_postal = "Code commune", DRG = "Segment DRG")
...
frequentation_clean <- frequentation %>%
  ...
  rename(Gare = "Nom de la gare", UIC = "Code UIC", code_postal = "Code postal", DRG = "Segmentation DRG")%>%
...
![](images/figure/carte-reseau-sncf-france2020.png "Carte du réseau SNCF en France (2020)"){width="600"} **Figure 2.** Carte du réseau SNCF en France (2020). **Source :** <https://www.sncf-connect.com/aide/le-reseau-sncf-en-france-et-en-europe>
...
ggplot(freq_departement, aes(x = Total_passagers, y = departement)) +
...
correspondance <- data.frame(
  departement = c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", 
...
**Analyse.** Comme le constat fait un peu plus haut, on observe que la région Île-de-France (composée des départements 75, 77, 78, 91, 92, 93, 94, 95) concentre la plupart des voyageurs. Ce constat est tout à fait correct étant donné que la région concentre environ 12 millions de personnes à l'année. On peut également souligner que le réseau SNCF désert la gare de Monaco-Monte-Carlo qui n'est pas comptabilisé dans les régions françaises. Nous avons donc dû l'intégrer manuellement.
...
departements_geojson <- st_read(url, quiet = TRUE)

departements_data <- departements_geojson %>%
  rename(departement = "code") %>%
  left_join(freq_departement, by = "departement")
...
> `Nous avons donc proposé d'étudier la question : Quel impact sur les voyageurs a eu la pandémie de Covid ?`

> `Nous supposons que la fréquentation était très très faible sur 2020`
...
total_par_annee <- frequentation_clean %>%
  group_by(Année) %>%
  summarise(Total_Voyageurs = sum(Voyageurs))


ggplot(total_par_annee, aes(x = Année, y = Total_Voyageurs, group = 1)) +
  ylim(0,3000000000) +
  geom_line() +  
  geom_point() +  
...
> `Réponse à la question : On distingue que la baisse a été de moitié en 2020. Le réseau a eu besoin de 2 ans pour revenir à la normale.`

Maintenant intéressons nous aux nombres de voyageurs par segment DRG.

> `Nous avons donc proposé d'étudier la question : Quelle fréquentation y a t il selon le type de gare ?`

> `Nous nous attendons à énormément de voyageurs dans les très grandes gares et beaucoup moins dans les autres`

```{r}
data_segmentation <- frequentation_clean %>%
  filter(Année == 2022) %>%
  group_by(DRG) %>%
  summarise(Total_Voyageurs = sum(Voyageurs)) %>%
  na.omit()

ggplot(data_segmentation, aes(x = DRG, y = Total_Voyageurs, fill = DRG)) +
  geom_bar(stat = "identity") +
  ylim(0,2000000000) +
  labs(title = "Répartition des voyageurs par segment DRG en 2022",
       x = "Segment DRG",
       y = "Nombre total de voyageurs",
       fill = "Segment DRG") +
  theme_minimal()
...
> `Réponse à la question : Les voyageurs sont très souvent présents dans des grandes gares. Cependant, il y a tout de même 57 millions de voyageurs passant par les petites gares. Il y a quasiment autant de voyageurs dans les gares moyennes et nationales. Cela fait echo aux motifs de déplacement.`
...
Combien d'objets perdus ont une gare affiliée

```{r}
obj_perdus_trouves <- obj_perdus_2019 %>%
  filter(is.na(gare) == FALSE)

count(obj_perdus_trouves)$n
```

Nous allons maintenant nous concentrés sur ces objets car cela indique qu'ils ont sûrement été rendus dans la gare.

```{r}
obj_perdus_trouves %>% 
  ggplot(aes(y=fct_rev(fct_infreq(type)))) +
  geom_bar(fill="blue4") +
  labs(title = "Nombre d'objets perdus en gare par type (2019)",
       y = "Type d'objet",
       x = "Nombre d'objets perdus en unité") +
  theme_minimal() +
  theme(
...
Combien y a-t-il d'objets rendus :
...
obj_trouves_2019 <- obj_trouves_clean %>%
  ...
  filter(date < ymd(20200101)) %>% 
  filter(date > ymd(20190101))
count(obj_trouves_2019)$n
```

On découvre donc qu'il y a plus du double d'objets perdus.

Comptons les objets rendus avec un UIC :

```{r}
obj_trouves_rendus <- obj_trouves_2019 %>%
  filter(is.na(UIC) == FALSE)

count(obj_trouves_rendus)$n
...
liste_UIC_trouves <- obj_trouves_rendus %>%
  select(UIC)
count(intersect(liste_UIC_perdus, liste_UIC_trouves))$n
```

Il semble que seulement 145 code UIC différents et après quelques recherches en ligne, le code UIC est l'ID des gares. Donc cette colonne ne nous est pas forcément utile.
...
> `Réponse à la question : Plus un objet semble de grande taille, plus celui-ci semble être retrouvé par son propriétaire. Les objets nécessaires sont aussi plus souvent retrouvés : articles médicaux, porte-monnaies`
...
> `Réponse à la question : Au vu de la croissance d'objets perdus importante de l'année dernière, on peut s'attendre à avoir plus d'objets perdus cet été (2024) que l'année précédente (2023). Le mois le plus important est celui de Juillet`
...
> `Nous imaginons que des gares très grandes (spatialement) ont plus de chances d'avoir des pertes d'objets.`
...
> `Réponse à la question : D'après notre visualisation, il semblerait qu'il y ait effectivement des gares à éviter pour ne pas perdre ses objets. Ici Granville apparait comme la gare à éviter selon notre étude. La taille d'une gare ne semble pas être un critère.`
