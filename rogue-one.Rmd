...
*`[TODO] Ajouter les tables modifiées`*
...
4.  Fréquentation des gares d'un même département (77)
...
Faisons un zoom sur ces départements et organisons les du plus fréquentés au moins fréquentés en fixant une limite minimum de 5 millions de voyageurs au total sur l'année 2022.

```{r}
freq_departement_plus <- frequentation_clean %>%
  filter(Voyageurs > 5000000) %>%
  filter(Année == "2022")

freq_departement_plus <- freq_departement_plus %>%
  group_by(Département) %>%
  summarize(Total_passagers = sum(Voyageurs))

ggplot(freq_departement_plus, aes(x = Total_passagers, y = reorder(Département, Total_passagers))) +
  geom_bar(stat = "identity") +
  labs(title = "Fréquentation des gares par département (2022)",
       subtitle = "< minimum 5.000.000 de voyageurs >",
       x = "Nombre total de passagers",
       y = "Département") +
...
**Analyse.** On constate que Paris est largement en tête de la fréquentation (département 75), suivi des départements de la région Ile-de-France (dans l'ordre : 93, 95, 92, 78, 91, 94). Le Rhône, région lyonnaise, (69) est devant le département 77 qui fait pourtant parti de la région Ile-de-France. Le Rhône (Lyon) semble donc avoir une importance dans le réseau ferroviaire français. On peut également se demander pourquoi le 77, est-il le département le moins fréquenté d'Ile-de-France? Enfin, on aurait pu également appliquer nos précédentes observations aux départements les moins fréquentés.
...
**Analyse.** Comme le constat fait un peu plus haut, on observe que la région Ile de France (composée des départements 75, 77, 78, 91, 92, 93, 94, 95) concentre la plupart des voyageurs. Ce constat est tout à fait correct étant donné que la région concentre environ 12 millions de personnes à l'année.
...
Complément : On réalise la même visualisation mais pour le département du Rhône.
...
-   Au sein des départements les gares les plus proches de la région parisienne semblent être les plus utilisées.
