...
```{r}
angouleme_data <- motif_depl %>%
  filter(`Gare enquêtée` == "Angoulême")
parisEst_data <- motif_depl %>%
  filter(`Gare enquêtée` == "Paris Est")
# Calculer le pourcentage moyen pour chaque motif de déplacement pour Angoulême
motif_angouleme <- angouleme_data %>%
  group_by(`Motif du déplacement`) %>%
  summarise(Pourcentage_moyen = mean(POURCENTAGE, na.rm = TRUE)) %>%
  arrange(desc(Pourcentage_moyen))
