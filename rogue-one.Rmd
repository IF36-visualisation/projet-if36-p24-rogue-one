...
```
**Qui est le voyageur moyen de la SNCF ?**
Il est également intéressant de recueillir des informations sur le voyageur moyen de la SNCF, 
En premier lieu nous allons voir les différentes catégories socio professionnelles.
...
ggplot(profil_moyen, aes(x = reorder(CSP, -Pourcentage_moyen), y = Pourcentage_moyen)) +
  geom_bar(stat = "identity") +
  coord_flip() + # Pour une meilleure lisibilité des étiquettes de CSP
...
>>>>>>> 0a11b07337ac41919572735f1c0ca28059535280
...
probas_types %>% 
  ggplot(aes(y=type_obj_trouves, x=Freq)) +
  geom_col() +
  labs(title = "Fréquence de rendu d'objet perdus par type",
       y = "Type d'objet",
       x = "Fréquence de retour")
```
