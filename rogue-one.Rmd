...
Réalisons maintenant une analyse multiscalaire afin d'avoir d'en avoir une meilleure vue d'ensemble et compréhension de la fréquentation de ce réseau.

Tout d'abord, on réalise une première vue globale pour faire ressortir les départements les plus fréquentées du réseau. On utilise pour cela des données discrètes en abscisse (Total_passagers) et ordinale en ordonnée (Département). Une visualisation avec un bar chart nous permet de faire cette comparaison.
...
**Analyse.** Comme le constat fait un peu plus haut, on observe que la région Ile de France (composée des départements 75,77, 78, 91, 92, 93, 94) concentre la plupart de la fréquentation. Ce constat est tout à fait correct étant donné que la région concentre environ 12 millions de personnes.
...
1.  Explorer la répartition par âge des passagers
2.  Catégorie socio professionnelle
3.  Motif de déplacement
...
obj_perdus_date_type2 %>%
  ...
  facet_grid(rows = vars(year(date))) +
  geom_bar() +
  labs(title = "Nombre d'objets perdus par mois et par année", y = "Nombre d'objets perdus", x = "Mois") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = rel(2)),
    axis.text.y = element_text(size = rel(0.5))
